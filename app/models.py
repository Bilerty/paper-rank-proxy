...
class JournalRankCache(Base):
    ...
    official_rank_all_json: Mapped[str | None] = mapped_column(Text)
    official_rank_select_json: Mapped[str | None] = mapped_column(Text)
    custom_rank_json: Mapped[str | None] = mapped_column(Text)
    sci: Mapped[str | None] = mapped_column(String(64))
    ssci: Mapped[str | None] = mapped_column(String(64))
    cas_zone: Mapped[str | None] = mapped_column(String(128))
    cas_small: Mapped[str | None] = mapped_column(String(128))
    cas_top: Mapped[str | None] = mapped_column(String(64))
    impact_factor: Mapped[float | None] = mapped_column(Float)
    five_year_if: Mapped[float | None] = mapped_column(Float)
    ei: Mapped[str | None] = mapped_column(String(64))
    cscd: Mapped[str | None] = mapped_column(String(64))
    pku_core: Mapped[str | None] = mapped_column(String(64))
    cssci: Mapped[str | None] = mapped_column(String(64))
